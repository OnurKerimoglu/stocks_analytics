...
- **set_push_env** *(A, B)*:  this task reads the configuration parameter `env`, pushes it to `Xcom`, loads the configs under [Docker/airflow/config](Docker/airflow/config) according to the specified environment and returns the bucket, dataset and table names in a dictionary, which are used in downstream tasks. 
